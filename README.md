...
```text
bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.25.nupkg
```

## Install Locally

```powershell
dotnet new install .\bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.25.nupkg
```
...
```text
artifacts\vsix\EnhancedAspireStarter.VisualStudio.0.1.25.vsix
```
...
```powershell
.\scripts\Update-Template.ps1 -SourceRepository C:\Aspire\Starter -TemplateVersion 0.1.25
```
...
```powershell
dotnet new install .\bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.25.nupkg --force
