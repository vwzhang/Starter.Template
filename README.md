...
```text
bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.32.nupkg
```

## Install Locally

```powershell
dotnet new install .\bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.32.nupkg
```
...
```text
artifacts\vsix\EnhancedAspireStarter.VisualStudio.0.1.32.vsix
```
...
```powershell
.\scripts\Update-Template.ps1 -SourceRepository C:\Aspire\Starter -TemplateVersion 0.1.32
```
...
```powershell
dotnet new install .\bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.32.nupkg --force
