...
```text
bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.30.nupkg
```

## Install Locally

```powershell
dotnet new install .\bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.30.nupkg
```
...
```text
artifacts\vsix\EnhancedAspireStarter.VisualStudio.0.1.30.vsix
```
...
```powershell
.\scripts\Update-Template.ps1 -SourceRepository C:\Aspire\Starter -TemplateVersion 0.1.30
```
...
```powershell
dotnet new install .\bin\Release\Vwzhang.AspireAdminStarter.Templates.0.1.30.nupkg --force
